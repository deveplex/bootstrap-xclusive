...
(function (factory) {
    ...
    Dialog.prototype.defaults = {
        ...
        template: ['<div class="modal" tabindex="-1">', '<div class="modal-content">', '<div class="modal-header">', '<button type="button" class="cysicons close" style="border: 0;font-size: 1em;" data-dismiss="modal" aria-label="Close" aria-hidden="true"> &times;</button>', '<div class="modal-title" id="modal_title"></div>', '</div>', '<div class="modal-body"></div>', '<div class="modal-footer"></div>', '</div>', '</div>'].join('')
    };
    ...
    Dialog.prototype.initialize = function () {
        ...
        //modal
        $element.detach().show().data('bs.' + this.type, this).data('bs.template', $container);
        ...
        if (that.options.buttons && that.options.buttons.length > 0) {
            $.each(that.options.buttons, function (ix, it) {
                ...
                var $btn = $('<button type="button" class=""></button>').addClass(it.classes).text(it.text);
    ...
    function Plugin(options, _relatedTarget) {
        ...
        this.each(function () {
            ...
            var optionset = $.extend({}, Dialog.prototype.defaults, $this.data(), (typeof options === 'undefined' ? 'undefined' : _typeof(options)) === 'object' && options);

            if (!data && /destroy|hide/.test(optionset)) return;
            if (!data) $this.data('bs.dialog', data = new Dialog(this, optionset));

            if (typeof options === 'string') $container = data[options](_relatedTarget);else if (optionset.show) $container = data.show(_relatedTarget);else $container = data.hide(_relatedTarget);
        });
        return $container;
    }
