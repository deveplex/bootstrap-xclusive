...
(function (factory) {
    ...
    Table.prototype.initBody = function () {
        ...
        $.each(that.options.data, function (ix, row) {
            ...
            if (that.options.onRowClick) $tr.on('click', function () {
                    ...
                    case 'radiobox':
                        {
                            ...
                            var radioboxTemplate = $(['<div class="radiobox-item ' + radiobox.id + '">', '<label class="radio-box circle"><input type="radio" name="' + radiobox.name + '" id="' + radiobox.id + '_' + d.getTime() + '" class="" data-options="{iconFont:\"glyphicon glyphicon-ok\"" /></label>', '<label for="' + radiobox.id + '_' + d.getTime() + '" class=""></label>', '</div>'].join(''));
                            if (radioboxOptions.iconFont) radioboxTemplate.find('input[type=radio]').data('options', radioboxOptions);
                            radioboxTemplate.on('click', function (e) {
                                e.stopPropagation();
                            });
                    ...
                    case 'checkbox':
                        {
                            ...
                            var checkbokTemplate = $(['<div class="checkbox-item ' + checkbok.id + '">', '<label class="check-box rectangle"><input type="checkbox" id="' + checkbok.id + '_' + d.getTime() + '" class="" data-options="{iconFont:\"glyphicon glyphicon-ok\"" /></label>', '<label for="' + checkbok.id + '_' + d.getTime() + '" class=""></label>', '</div>'].join(''));
                            if (checkboxOptions.iconFont) checkbokTemplate.find('input[type=checkbox]').data('options', checkboxOptions);
                            checkbokTemplate.on('click', function (e) {
                                e.stopPropagation();
                            });
    ...
    function Plugin(options, _relatedTarget) {
        return this.each(function () {
            ...
            var optionset = $.extend({}, Table.prototype.defaults, $this.data(), (typeof options === 'undefined' ? 'undefined' : _typeof(options)) === 'object' && options);

            if (!data && /destroy|hide/.test(optionset)) return;
            if (!data) $this.data('bs.table', data = new Table(this, optionset));

            if (typeof options === 'string') data[options](_relatedTarget);else if (optionset.append) data.append(_relatedTarget);else data.append(_relatedTarget);
        });
    }
