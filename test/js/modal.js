...
(function (factory) {
    ...
    Modal.prototype.initialize = function () {
        ...
        //modal
        $backdrop.detach().addClass('in');

        $element.detach().css({ display: 'none' }).data('bs.' + this.type, this).data('bs.backdroptemplate', $backdrop);

        if (!$element.hasClass('modal')) {
            $element.addClass('modal');
        }
        if (that.options.classes) {
            $element.addClass(that.options.classes);
        }
        ...
        $element.on('hide.bs.modal', function (e) {
            ...
            if ($parent && $parent.length > 0) {
                ...
                if (children && children.length > 0) {
                    if (index === 0) $self.prependTo($parent);else $self.insertAfter($(children[index - 1]));
                } else $self.appendTo($parent);
            }
        });
    };
    ...
    function Plugin(options, _relatedTarget) {
        return this.each(function () {
            ...
            var optionset = $.extend({}, Modal.defaults, $this.data(), (typeof options === 'undefined' ? 'undefined' : _typeof(options)) === 'object' && options);
